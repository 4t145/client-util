...
I basically copied the code from the reqwest crate and pasted it here, and modify it to work with the hyper crate.
The source file is here: https://github.com/seanmonstar/reqwest/blob/master/src/async_impl/multipart.rs
...
use crate::body::{empty, full, stream, Body};
...
impl Form {
    ...
    pub(crate) fn stream(mut self) -> Body {
        ...
        let start = Box::pin(self.part_stream(name, part))
            as Pin<Box<dyn Stream<Item = Result<Frame<Bytes>, BoxError>> + Send>>;
        ...
        Body::new(new_stream(stream.chain(last)))
    }
    ...
    {
        // start with boundary
        let boundary = stream::once(future::ready(Ok(Frame::data(Bytes::from(format!(
            "--{}\r\n",
            self.boundary()
        ))))));
        // append headers
        let header = stream::once(future::ready(Ok({
            ...
            Frame::data(Bytes::from(h))
        })));
        // then append form data followed by terminating CRLF
        boundary
            .chain(header)
            .chain(BodyStream::new(part.value))
            .chain(stream::once(future::ready(Ok(Frame::data(
                Bytes::from_static(b"\r\n"),
            )))))
    }
...
mod tests {
    ...
    fn stream_to_end() {
        let mut form = Form::new()
            .part(
                "reader1",
                Part::stream(stream(stream::once(future::ready::<
    ...
    fn correct_content_length() {
        ...
        let stream_data = stream_data
            .chunks(3)
            .map(|c| Ok::<_, BoxError>(Frame::data(Bytes::from(c))));
        let the_stream = futures_util::stream::iter(stream_data);

        let bytes_data = b"some bytes data".to_vec();
        let bytes_len = bytes_data.len();

        let stream_part = Part::stream_with_length(stream(the_stream), stream_len as u64);
